...
function calculateShearForceAndBendingMoment(spans, spanLengths, loadTypes, loadValues, reactions) {
    const shearForces = [];
    const bendingMoments = [];

    for (let i = 0; i < spans; i++) {
        const spanLength = spanLengths[i];
        let shearForce = reactions[i]; // Start shear force at the left reaction
        let bendingMoment = 0;
        const loads = loadValues[i];

        for (let j = 0; j < loads.length; j++) {
            if (loadTypes[i][j] === "point") {
                const loadPosition = spanLength / 2;
                shearForce -= loads[j];
                bendingMoment += loads[j] * (spanLength - loadPosition);
            } else if (loadTypes[i][j] === "udl") {
                shearForce -= loads[j] * spanLength;
                bendingMoment += (loads[j] * spanLength ** 2) / 8; // Maximum moment for UDL
            }
        }

        shearForces.push(shearForce);
        bendingMoments.push(bendingMoment);
    }

    return { shearForces, bendingMoments };
}
